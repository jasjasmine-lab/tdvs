...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if attn_mask is not None:
            attn += attn_mask.unsqueeze(dim=0)

        attn_soft = F.softmax(attn, dim=-1)

        if self.dropout:
            attn_soft = F.dropout(attn_soft, p=self.dropout)

        out = (attn_soft @ v).transpose(1, 2).reshape(B, N, C)
...
#
#         q = q.view(tgt_len, bsz * self.heads, self.dim_head).transpose(0, 1)
#
#         k = k.view(k.shape[0], bsz * self.heads, self.dim_head).transpose(0, 1)
#
#         v = v.view(v.shape[0], bsz * self.heads, self.dim_head).transpose(0, 1)
#
#         b, _, E = q.shape
#
...
#             # out = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=attn_mask, op=self.attention_op)
#             attn_output_weights = torch.bmm(q_scaled, k.transpose(-2, -1))
#         else:
#             attn_mask = attn_mask.repeat(q.shape[0], 1, 1)
#             # out = xformers.ops.memory_efficient_attention(q, k, v, attn_bias=attn_mask, op=self.attention_op)
...
class MemoryEfficientSelfAttention(nn.Module):
    ...
    def forward(self, x, k, value, attn_mask=None):
        q = self.to_q(x)
        k = self.to_k(k)
        v = self.to_v(value)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = MultiheadAttention(hidden_dim, nhead, dropout=dropout)
        # self.self_attn = MemoryEfficientSelfAttention(hidden_dim, nhead, dropout=dropout)
        # self.self_attn = SelfAttention0(hidden_dim, nhead, dropout=dropout)
...
if __name__ == "__main__":
    self_attn1 = nn.MultiheadAttention(320, 8, dropout=0.1).cuda()
    # self_attn2 = SelfAttention0(320, 8, dropout=0.1).cuda()
    self_attn3 = SelfAttention(320, 8, dropout=0.1).cuda()

    # x = torch.randn(12, 1024, 320).cuda()
    x = torch.randn(1024, 12, 320).cuda()

    # o1 = self_attn1(x, x, x)[0]
    # o2 = self_attn2(x, x, x)
    # o2 = self_attn3(x, x, x)

...
def log_local(images, filenames, save_path):
    root = os.path.join(save_path)
    for i in range(len(filenames)):
        name = filenames[i][-7:-4]
        for k in ['reconstruction', 'samples']:
            image = (images[k][i].squeeze().clamp(-1,1) * 0.5 + 0.5) * 255.0
            image = image.permute(1, 2, 0).to('cpu').numpy().astype("uint8")
            filename = "{}-{}.jpg".format(name, k)
            path = os.path.join(root, filenames[i][:-7],filename)
            os.makedirs(os.path.split(path)[0], exist_ok=True)
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.imwrite(path, image)


def cal_anomaly_map(fs_list, ft_list, out_size=224, amap_mode='mul', dis='eucl'):
    if amap_mode == 'mul':

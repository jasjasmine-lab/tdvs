        ...
        fileinfos.append(
            {
                ...
                "clsname": result['clsname'][i],
            }
        )
    preds = np.concatenate(np.asarray(result['pred']), axis=0)  # N x H x W
    masks = np.concatenate(np.asarray(result['mask']), axis=0)  # N x H x W
...
class EvalPerPixelAUC:
    ...
    def eval_auc(self):
        ...
        # plt.plot(fpr, tpr, color='blue', lw=2, label="AUROC")
...
def save_metrics(ret_metrics, evl_metrics, path):
    clsnames = [k.rsplit("_", 2)[0] for k in ret_metrics.keys()]
    clsnames = set(clsnames)
    clsnames = list(set(clsnames) - set(["mean"])) + ["mean"]
    evl_metrics_list = [evl['name'] for evl in evl_metrics['auc']]

    evl_metrics_list.sort()

    result_data = {}
    for cls in clsnames:
        result_data[cls] = {}
        for evl in evl_metrics_list:
            result_data[cls][evl] = round(ret_metrics[f'{cls}_{evl}_auc'], 3)
